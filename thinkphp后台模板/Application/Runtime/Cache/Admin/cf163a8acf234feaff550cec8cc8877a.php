...
<block name="body">
	...
	<div class="content">
		<div class="Config_nav">
			<ul>
				<li><a class="on" href="<?php echo U('Download/index');?>">下载管理</a></li>
				<li><a href="<?php echo U('Download/add');?>">添加下载</a></li>
				<li><a href="<?php echo U('Download/index');?>">下载评论</a></li>
                <li><a href="<?php echo U('Download/recycle');?>">回收站</a></li>
			</ul>
		</div>
		<div class="Search">
			<form action="" method="get">
				<div class="JianSuo">
					<select name="catid">
						<option>选择栏目</option>
						<?php if(is_array($cate_list)): foreach($cate_list as $k=>$vo): ?><option value="<?php echo ($vo["catid"]); ?>" <?php if($vo['ispart']==1 or $vo['ismodel']!=4){echo "disabled='disabled'";}?>><?php echo str_repeat("└─",$vo['level']); echo ($vo["catname"]); ?></option><?php endforeach; endif; ?>
					</select>
				</div>
				<div class="JianSuo">
		...
		<form action="<?php echo U('Download/sortBatch');?>" name="myform" id="myform" method="post">
			<div class="List">
				...
				<?php if(is_array($Download_list)): foreach($Download_list as $k=>$vo): ?><div class="ProList">
						<ul>
							<li class="ID"><?php echo ($vo["id"]); ?></li>
							<li class="Name"><a href="<?php echo U('Download/edit',array('id'=>$vo['id']));?>"><?php echo ($vo["title"]); ?></a></li>
							...
							<li class="Oper">
								<a href="<?php echo U('Articlecomment/index',array('articleid'=>$vo['id']));?>">评论</a> | <a href="<?php echo U('Download/edit',array('id'=>$vo['id']));?>">编辑</a> | <a href="<?php echo U('Download/delete',array('id'=>$vo['id']));?>">删除</a>
							</li>
						</ul>
